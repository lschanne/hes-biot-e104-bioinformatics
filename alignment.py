...
def local_alignment(
    ...
    """
    Given two DNA sequences, compute the maximum local alignment score via the
    Smith-Waterman method. Return the score and all alignments that yield
    that score.
    ...
    -------
    [0]: int
        The maximum local alignment score achievable
    [1]: List[Tuple[str]]
        All possible alignments that yield the local global alignment score
        [0]: The alignment of the seq1 DNA sequence
        [1]: The alignment of the seq2 DNA sequence
    """
    ...
    scores = np.zeros([len1 + 1, len2 + 1]).astype(int)
    paths = [[None for _ in range(len2)] for _ in range(len1)]
    ...
    def _build_alignments(alignment1, alignment2, idx1, idx2):
        if idx1 <= 0 and idx2 <= 0:
            ...
            return
        these_paths = paths[idx1][idx2]
        if not any(these_paths):
            alignments.append(
                (seq1[idx1] + alignment1, seq2[idx2] + alignment2)
            )
            return
        if these_paths[UP]:
            _build_alignments(
                GAP + alignment1,
                seq2[idx2] + alignment2,
                idx1,
                idx2 - 1,
            )
        if these_paths[LEFT]:
            _build_alignments(
                seq1[idx1] + alignment1,
                GAP + alignment2,
                idx1 - 1,
                idx2,
            )
    ...
    if best_pos:
        for iii, jjj in best_pos:
            _build_alignments("", "", iii, jjj)
    else:
        alignments = [("", "")]
...
if __name__ == "__main__":
    ...
    # TODO add these to a test suite along with more robust tests
    ...
    print(alignments)
    print(alignment_score == 1)
    print(alignments == [('AACTT', '-ACAT'), ('AACTT', 'TACAT')])

    alignment_score, alignments = local_alignment(
        seq1="AACTTA",
        seq2="ACATG",
        match_score=3,
        mismatch_penalty=-1,
    ...
    print(alignment_score)
    print(alignments)
    print(alignment_score == 8)
    print(alignments == [("ACTT", "ACAT")])
